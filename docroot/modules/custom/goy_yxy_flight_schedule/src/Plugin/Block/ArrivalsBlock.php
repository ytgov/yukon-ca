 ...
 * @Block(
 *   id = "arrivals_block",
 *   admin_label = @Translation("Arrivals Block"),
 * )
 */
class ArrivalsBlock extends BlockBase implements ContainerFactoryPluginInterface {
  /**
  ...
  public function build() {
    ...
    $districtUrl = $config->get('goy_yxy_flight_schedule_xml_source_path') . '/' . $config->get('goy_yxy_flight_schedule_arrivals_file_name');
      ...
      if (count($xml->Arrival) > 0) {
        foreach ($departure as $xml_flight) {
          ...
          if ($heading == 'departure') {
            $row[] = [
              'data' => intval((string) $xml_flight->TempCel) . '&degC',
              'class' => 'temperature',
            ];
          }
