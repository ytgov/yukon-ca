 ...
 * process:
 *   field_image_gallery:
 *     plugin: yukon_paragraph_image_gallery
 * @endcode
 *
 * @see \Drupal\migrate\Plugin\MigrateProcessInterface
 *
 * @MigrateProcessPlugin(
 *   id = "yukon_paragraph_image_gallery",
 * )
 */
    ...
    if (!empty($results)) {
      ...
      foreach ($results as $result) {
        ...
        $imageMedia = $this->entityTypeManager->getStorage('media')->loadByProperties([
          ...
          'field_media_image_1' => ['target_id' => $result->field_slide_image_fid],
        ]);
        ...
        $paragraph->field_description_plain->value = $result->field_description_value;
        $paragraph->field_link = [
          'uri' => 'internal:/' . $result->field_link_url,
          'title' => $result->field_link_title,
        ];
        $paragraph->field_image->target_id = $imageMedia->id();
        $paragraph->save();
